...
strong_actor_ptr actor_registry::get_impl(string_view key) const {
...
void actor_registry::put_impl(std::string key, strong_actor_ptr value) {
  if (value == nullptr) {
    erase(key);
    return;
  }
...
void actor_registry::erase(string_view key) {
